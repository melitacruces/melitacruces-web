...
export default function Education() {
  return (
    <section className="container section" id="education" aria-labelledby="education-heading">
      <div className="section-inner">
        <div className="section-header">
          <SectionTitle id="education-heading">Formación y Certificaciones</SectionTitle>
        </div>
        <div className="mt-10 w-full max-w-5xl mx-auto flex flex-col gap-10">
          <div className="flex flex-col gap-6">
            {portfolioData.education.map((education) => (
              <Panel as="article" key={education.degree} className="w-full relative overflow-hidden group">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-6">
                  <div>
                    <h3 className="text-xl sm:text-2xl font-bold mb-1 group-hover:text-white transition-colors">
                      {education.degree}
                    </h3>
                    ...
                    <h4 className="text-base sm:text-lg text-foreground/70 font-medium">
                      {education.institution}
                    </h4>
                  </div>
                  <span className="text-xs sm:text-sm font-semibold px-3 py-1.5 bg-tag/10 text-tag-light rounded-2xl border border-tag/20 w-max flex items-center gap-2 tracking-wide flex-none">
                ...
                {education.certificates?.length > 0 && (
                  <div className="flex flex-wrap gap-3 mt-0 pt-6 border-t border-white/15 relative z-20">
                    {education.certificates.map((certificate) => (
                      <a
                        ...
                        className="text-xs font-semibold px-4 py-2 bg-background border border-white/15 rounded-2xl text-foreground/75 hover:bg-white/10 hover:text-white hover:border-white/35 transition-all flex items-center gap-2"
                      >
