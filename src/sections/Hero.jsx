      ...
      <div className="container relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 w-full relative">
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left flex-1 px-0">
            <h1
              id="hero-heading"
              className="text-4xl sm:text-6xl md:text-7xl lg:text-[5rem] font-bold mb-4 tracking-tight leading-[1.15] lg:-ml-1 lg:pr-2"
            >
              <span className="block text-foreground">Hola, soy</span>
              <span className="block bg-gradient-to-r from-accent to-white bg-clip-text text-transparent pr-1 lg:pr-2">
          ...
          <div className="flex-1 w-full max-w-sm lg:max-w-md relative mx-auto lg:mx-0">
            <div className="relative aspect-square w-full">
              <motion.div
                ...
                className="absolute inset-0 bg-gradient-to-br from-white/50 to-white/10 rounded-2xl opacity-40 blur-sm"
              />
              <motion.div
                animate={{ rotate: [-3, -9, -3], scale: [1.05, 1.1, 1.05] }}
                transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }}
                className="absolute inset-0 bg-gradient-to-tl from-white/25 to-transparent rounded-2xl opacity-40 blur-sm"
              />

              <div className="relative w-full h-full rounded-2xl border-2 border-white/25 overflow-hidden bg-background shadow-[0_0_30px_rgba(255,255,255,0.18)] group">
                <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-transparent to-transparent z-10 opacity-60" />
                <img
                  src="/images/melitacruces-blanco-negro.jpg"
                  ...
                  onError={(event) => {
                    ...
                    event.currentTarget.src =
                      'https://ui-avatars.com/api/?name=Luis+Melita&size=512&background=050505&color=FFFFFF&rounded=false&bold=true';
                  }}
                />
              </div>
            </div>
